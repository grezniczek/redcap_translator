...
class Downloader {
    ...
    private static function get_package($m, $mode, $version) {
        ...
        if ($mode == "package-get-strings") {
            $result = $m->get_strings_from_zip($doc_id, $version);
            if (is_array($result)) {
                ...
                print $m->strings_to_ini($result, "; English.ini - REDCap v$version [generated by REDCap Translation Assistant EM]");
            }
            else {
    ...
    private static function get_translation($m, $mode, $name, $based_on) {
        ...
        if ($mode == "translation-get-json") {
            ...
            $json = $m->sanitize_translation($lang);
        ...
        if ($mode == "translation-get-ini") {
            ...
            header('Content-Type: text/plain; name="'.$ini_name.'"');
            header('Content-Disposition: attachment; filename="'.$ini_name.'"');
            print $m->strings_to_ini($result, "; $name.ini - $localized_name [generated by REDCap Translation Assistant EM]");
        }
    }
    ...
    private static function get_metadata($m, $version, $based_on, $add_code) {
        ...
        $result = $m->generate_metadata($doc_id, $version, $based_on, $add_code);

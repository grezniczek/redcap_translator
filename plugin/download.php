...
class Downloader {
    ...
    private static function get_translation($mode, $name, $based_on) {
        ...
        if ($mode == "translation-get-ini") {
            ...
            $result = [];
            $meta = self::$m->get_metadata_file($based_on);
            foreach ($translation["strings"] as $key => $item) {
                if ($item["do-not-translate"]) continue;
                $string_meta = $meta["strings"][$key] ?? null;
                if ($string_meta) {
                    $text = $item["translations"][$string_meta["hash"]] ?? null;
                    if ($text == null) {
                        $text = $item["translations"][""] ?? null;
                    }
                    if ($text == null && count($item["translations"])) {
                        $text = end($item["translations"]);
                    }
                }
                $result[$key] = $text;
            }
            header('Content-Type: text/plain; name="'.$ini_name.'"');
            header('Content-Disposition: attachment; filename="'.$ini_name.'"');
            print self::$m->strings_to_ini($result, [
                "; $name.ini - $localized_name based on $based_on metadata [generated by REDCap Translation Assistant EM]",
                "; Translation last updated: {$translation["timestamp"]}",
                "; Metadata last updated: {$meta["timestamp"]}",
            ]);
            exit;
        }
